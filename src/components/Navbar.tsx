  ...
  return (
    <nav className={`py-4 fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
      ...
      <div className="container-custom flex items-center justify-between">
        {/* Logo */}
        <Link to={getLanguagePrefix()} className="text-2xl font-bold text-faran-brown">
        ...
        <div className="md:hidden flex items-center gap-4">
          ...
          <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
            <SheetTrigger asChild>
              <button onClick={toggleMenu} className="text-faran-brown hover:text-faran-gold focus:outline-none">
                <Menu className="h-6 w-6" />
              </button>
            </SheetTrigger>
            <SheetContent side={isRTL ? "left" : "right"} className="bg-white text-faran-brown">
              <SheetHeader>
                <SheetTitle className="text-faran-gold">{isRTL ? "القائمة" : "Navigation"}</SheetTitle>
                <SheetDescription className="text-faran-lightText">
              ...
              <div className="flex flex-col space-y-4 mt-4">
                {navItems.map((item, index) => (
                  <Link
                    key={index}
                    to={item.path}
                    className={`text-faran-brown hover:text-faran-gold transition-colors duration-300 block py-2 ${
                      location.pathname === item.path ? 'text-faran-gold' : ''
                    }`}
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 text-faran-brown hover:text-faran-gold transition-colors duration-300 py-2"
